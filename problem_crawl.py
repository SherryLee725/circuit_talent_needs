...
def update_data(job_id, job_detail):
    conn = connection()
    cursor = conn.cursor()
    sql_str =  """update `problem_clean` set job_detail='{}' WHERE job_id='{}'""".format(job_detail,job_id)
...
def get_job_id_state():
    conn = connection()
    cursor = conn.cursor()
    sql_str = 'SELECT * FROM `problem_clean` '
    cursor.execute(sql_str)
    results = cursor.fetchall()
    cursor.close()
    conn.close()
...
def get_url(job_id):
    ...
    conn = connection()
    cursor = conn.cursor()
    sql_str = "SELECT job_href FROM `job_shortmess_words` where job_id = '"+str(job_id)+"'"
    cursor.execute(sql_str)
    results = cursor.fetchall()
    cursor.close()
    conn.close()
    if len(results)>0:
        job_href = results[0]['job_href']
    return job_href
...
def get_job_detail():
    ...
    for j in job_id_lists:
        ...
        try:
            ...
            job_detail, job_id = read_html(job_id, page_text)
            if job_detail is not None:
                # insert_data(job_lists)
                update_data(job_id,job_detail)
                # return
